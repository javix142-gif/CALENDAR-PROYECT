...
public final class AndroidTvRemoteClient {
    ...
    private static final int ACTIVE_FEATURES = 1 | 2 | 32 | 64 | 512; // ping, keys, power, volume, app-link
        ...
        io.execute(() -> {
            ...
            try {
                ...
                Proto.writeFrame(out, outer(Proto.fieldBytes(10,
                        Proto.concat(
                                Proto.fieldString(1, "atvremote"),
                                Proto.fieldString(2, "Sala Remote")
                        ))));
    ...
    public void finishPairing(String code, String host, Runnable paired) {
        io.execute(() -> {
            try {
                ...
                byte[] secretMsg = Proto.fieldBytes(1, secret);
                Proto.writeFrame(pairingSocket.getOutputStream(), outer(Proto.fieldBytes(40, secretMsg)));
    ...
    public void sendKey(int keyCode) {
        io.execute(() -> {
            try {
                ...
                postError("No se pudo enviar el comando");
            }
        });
    }
        ...
        try {
            ...
            while (!socket.isClosed()) {
                ...
                if (Proto.first(m, 1) != null) {
                    byte[] deviceInfo = Proto.concat(
                            Proto.fieldVarint(3, 1),
                            Proto.fieldString(4, "1"),
                            Proto.fieldString(5, "cl.javier.salaremote"),
                            Proto.fieldString(6, "1.0.0")
                    );
                    ...
                    byte[] active = Proto.fieldVarint(1, ACTIVE_FEATURES);
                    synchronized (sendLock) { Proto.writeFrame(out, Proto.fieldBytes(2, active)); }
    ...
    private SSLSocket createSocket(String host, int port) throws Exception {
        ...
        SSLSocketFactory f = ssl.getSocketFactory();
    ...
    private void ensureIdentity() throws Exception {
        if (identity == null) identity = ClientIdentity.loadOrCreate(context);
    }

    private static byte[] outer(byte[] payload) {
        return Proto.concat(Proto.fieldVarint(1, 2), Proto.fieldVarint(2, 200), payload);
    }
    ...
    private static byte[] makeSecret(String pin, X509Certificate client, X509Certificate server) throws Exception {
        ...
        sha.update(unsigned(c.getModulus()));
        sha.update(unsigned(c.getPublicExponent()));
        sha.update(unsigned(s.getModulus()));
        sha.update(unsigned(s.getPublicExponent()));
        sha.update(hexToBytes(pin.substring(2)));
        byte[] result = sha.digest();
        int first = Integer.parseInt(pin.substring(0, 2), 16);
        if ((result[0] & 0xff) != first) throw new IllegalArgumentException("Código incorrecto");
        return result;
    }
    ...
    private void closeRemote() {
        SSLSocket s = remoteSocket; remoteSocket = null; remoteOut = null;
        if (s != null) try { s.close(); } catch (IOException ignored) {}
    }
    private void closePairing() {
        SSLSocket s = pairingSocket; pairingSocket = null; pairingServerCert = null;
        if (s != null) try { s.close(); } catch (IOException ignored) {}
    }
    ...
    private static String shortMessage(Throwable t) {
        String m = t.getMessage(); return (m == null || m.isBlank()) ? t.getClass().getSimpleName() : m;
    }
}
